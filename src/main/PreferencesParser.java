...
public class PreferencesParser {
	public void loadPreference(String path,String fileName){
		try {
			NodeList root = buildDocument(path);
			NodeList preferences = getNode("List", root).getChildNodes();
			NodeList filePreferences = getNode(fileName, preferences).getChildNodes();
