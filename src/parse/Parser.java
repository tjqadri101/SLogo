...
public class Parser {
	...
	public void createFunctionsAndVariables(String s) {
			...
			for (String thisFunction : functionString) {
				if (!thisFunction.equals("")) {
					...
					while (wordsInFunction[functionNameIndex].equals("")
						...
						functionNameIndex++;
						functionContentStartIndex += wordsInFunction[functionNameIndex]
								.length() + 1;
					}
					String functionName = wordsInFunction[functionNameIndex];
					String content = thisFunction
							.substring(functionContentStartIndex);
		...
		for (int i = 0; i < words.length; i++) {
			if (words[i].charAt(0) == ':') {
				...
				VariableNode vn = new VariableNode(myTurtle,
	...
	public AbstractNode createTree(Function function, Turtle turtle)
		...
		int beginIndex = 0;
		while (function.getContent().charAt(beginIndex) == ' '
				|| function.getContent().charAt(beginIndex) == '[') {
			beginIndex++;
		}
		int endIndex = function.getContent().length() - 1;
		while (function.getContent().charAt(endIndex) == ' '
				|| function.getContent().charAt(endIndex) == '[') {
			endIndex--;
		}
		function.setContent(function.getContent().substring(beginIndex,
				endIndex + 1));
		...
		while (!queue.isEmpty()) {
			if (currentNode instanceof NumberNode
				...
				if (!currentNode.allowsTwoChildren()
						|| (currentNode.allowsTwoChildren() && currentNode
								.getChildren().size() == 2)) {
					currentNode = currentNode.getParent();
				}
			}
			...
			if (currentNode instanceof IfElseNode) {
				...
				// create condition for condition left; condition right is the
				// opposite of condition left
				currentNode = bnLeft.getLeftNode();
			}
			if (currentNode instanceof IfNode
					|| currentNode instanceof RepeatNode) {
				// create 1 block node
			...
			if (nextWord.equals("[")) {
				// if the parent node is a repeat node or an if node, or the
				// parent of the parent node is an if else node
				if (currentNode.getParent() instanceof RepeatNode
						|| currentNode.getParent() instanceof IfNode) {
					currentNode = currentNode.getParent().getRightNode(); // go
																			// to
																			// block
			...
			if (nextWord.equals("[")) {
				...
				if (currentNode.getParent() instanceof RepeatNode
					...
					currentNode = currentNode.getParent().getRightNode(); // go
						...
						&& currentNode.getLeftNode().getChildren().size() >= 2) {
					// ifelse node's left block is completed --> go to right
					// block
			...
			if (currentNode.getLeftNode() == null) {
				currentNode.setLeftNode(nextNode);
					...
					&& currentNode.allowsTwoChildren()) {
				currentNode.setRightNode(nextNode);
			} else if (currentNode.getRightNode() != null
					&& currentNode.allowsMoreThanTwoChildren()) {
